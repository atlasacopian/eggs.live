...
export default async function Home() {
  try {
    ...
    const latestRegularAvg = await safeQuery(() => 
      prisma.averagePrice.findFirst({
        where: { eggType: "regular" },
        orderBy: { date: "desc" },
      })
    );

    const latestBrownAvg = await safeQuery(() => 
      prisma.averagePrice.findFirst({
        where: { eggType: "brown" },
        orderBy: { date: "desc" },
      })
    ...
    const latestOrganicAvg = await safeQuery(() => 
      prisma.averagePrice.findFirst({
    ...
    const latestRegularDozen = await safeQuery(() => 
      prisma.dozenPrice.findFirst({
        where: { eggType: "regular" },
        orderBy: { date: "desc" },
      })
    );

    const latestBrownDozen = await safeQuery(() => 
      prisma.dozenPrice.findFirst({
        where: { eggType: "brown" },
        orderBy: { date: "desc" },
      })
    ...
    const latestOrganicDozen = await safeQuery(() => 
      prisma.dozenPrice.findFirst({
        where: { eggType: "organic" },
        orderBy: { date: "desc" },
      })
    ...
    // If we can't get any data, show an error message
      ...
      return (
        <main className="flex min-h-screen flex-col items-center justify-center p-4">
          <div className="bg-black p-6 rounded-lg text-green-400 font-mono max-w-md w-full">
            <h1 className="text-2xl mb-4">🥚 eggs.live</h1>
    ...
    return (
      <main className="container mx-auto p-4">
        <h1 className="text-4xl font-bold text-center my-8">🥚 eggs.live</h1>
